...
final class ReleaseManifestService
{
    ...
    public function latest(?string $target=null,bool $refresh=false): ?array
    {
        ...
        $manifest=$this->manifest($refresh);if(!$manifest)return null;
    ...
    {
        $release=$this->latest(null,$refresh);$current=ltrim(trim($currentVersion),'vV');
        ...
        $binary=$this->latest($target,false);if($binary){$status['url']=$binary['url'];$status['sha256']=$binary['sha256'];$status['size']=$binary['size'];}
        return $status;
    }
    private function release(bool $refresh=false): ?array
    {
        $cache=sys_get_temp_dir().'/vendo-gateway-firmware-release-'.sha1($this->releaseApiUrl).'.json';
        if(!$refresh&&is_file($cache)&&filemtime($cache)!==false&&time()-filemtime($cache)<$this->cacheSeconds){$data=json_decode((string)@file_get_contents($cache),true);if(is_array($data))return $data;}
    ...
    private function manifest(bool $refresh=false): ?array
    {
